/**
 * Start and stop g420 from Node.js.
...
module.exports = {
    ...
    configure: function (options) {
        this.bin = options.g420_bin || "g420";
        this.persist = options.persist || false;
        var f = copy(options.flags || options);
        this.network = f.networkid;
        f.datadir = f.datadir || join(process.env.HOME, ".fourtwenty-" + f.networkid);
        if (options.symlink) {
        ...
        if (options.account) {
            this.flags = this.flags.concat([
                "--fourtwentybase", options.account,
        ...
        if (this.configured) {
            ...
            if (!listeners.close) {
                listeners.close = function (code) {
                    if (code !== 2 && code !== 0) {
                        self.trigger(new Error("g420 closed with code " + code));
                    }
                };
            }
