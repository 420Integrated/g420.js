...
var fourtwentyrpc = require("fourtwentyrpc");
var g420 = require("../");
g420.debug = false;

var SYMLINK = join(process.env.HOME, "fourtwentylink");
var COINBASE = {
...
function runtests(options) {
    ...
    describe("fourtwentyrpc commands", function () {
        ...
        before(function (done) {
            ...
            g420.start(options, function (err, spawned) {
                if (err) return g420.stop(function () { done(err); });
                if (!spawned) return g420.stop(function () { done(new Error("where's the g420?")); });
                fourtwentyrpc.ipcpath = join(g420.datadir, "g420.ipc");
                done();
            });
        });
        ...
        describe("broadcast", function () {
            ...
            var test = function (t) {
                it(JSON.stringify(t.command) + " -> " + t.expected, function (done) {
                    this.timeout(TIMEOUT);
                    fourtwentyrpc.broadcast(t.command, function (res) {
            ...
            test({
                command: {
                    id: ++requests,
                    jsonrpc: "2.0",
                    method: "fourtwenty_protocolVersion",
                    params: []
                },
        ...
        describe("listening", function () {
            ...
            var test = function (t) {
                it(t.node + " -> " + t.listening, function (done) {
                    this.timeout(TIMEOUT);
                    fourtwentyrpc.listening(function (listening) {
        ...
        describe("version (network ID)", function () {
            ...
            var test = function (t) {
                it(t.node + " -> " + t.version, function (done) {
                    this.timeout(TIMEOUT);
                    fourtwentyrpc.version(function (version) {
        ...
        describe("Fourtwenty bindings", function () {

            it("fourtwenty('protocolVersion')", function (done) {
                fourtwentyrpc.fourtwenty("protocolVersion", null, function (res) {
                    if (res.error) return done(res);
            ...
            it("getGasPrice", function (done) {
                fourtwentyrpc.getGasPrice(function (res) {
                    if (res.error) return done(res);
            ...
            if (!process.env.CONTINUOUS_INTEGRATION) {
                ...
                it("blockNumber", function (done) {
                    fourtwentyrpc.blockNumber(function (res) {
                        if (res.error) return done(res);
                ...
                it("balance/getBalance", function (done) {
                    fourtwentyrpc.balance(coinbase, function (res) {
                        if (res.error) return done(res);
                        assert.isAtLeast(parseInt(res), 0);
                        fourtwentyrpc.getBalance(coinbase, function (r) {
                            if (r.error) return done(r);
                            assert.isAtLeast(parseInt(r), 0);
                            assert.strictEqual(r, res);
                            fourtwentyrpc.balance(coinbase, "latest", function (r) {
                                if (r.error) return done(r);
                                assert.isAtLeast(parseInt(r), 0);
                                assert.strictEqual(r, res);
                                fourtwentyrpc.getBalance(coinbase, "latest", function (r) {
                                    if (r.error) return done(r);
                                    assert.isAtLeast(parseInt(r), 0);
                                    assert.strictEqual(r, res);
                                    fourtwentyrpc.balance(coinbase, null, function (r) {
                                        if (r.error) return done(r);
                                        assert.isAtLeast(parseInt(r), 0);
                                        assert.strictEqual(r, res);
                                        fourtwentyrpc.getBalance(coinbase, null, function (r) {
                                            if (r.error) return done(r);
                                            assert.isAtLeast(parseInt(r), 0);
                                            assert.strictEqual(r, res);
                                            done();
                ...
                it("txCount/getTransactionCount", function (done) {
                    fourtwentyrpc.txCount(coinbase, function (res) {
                        if (res.error) return done(res);
                        assert(parseInt(res) >= 0);
                        fourtwentyrpc.pendingTxCount(coinbase, function (res) {
                            if (res.error) return done(res);
                            assert(parseInt(res) >= 0);
                            done();
                        });
            ...
            it("peerCount", function (done) {
                fourtwentyrpc.peerCount(function (res) {
                    if (res.error) return done(res);
                    assert(parseInt(res) >= 0);
                    done();
                });
            ...
            it("hashrate", function (done) {
                fourtwentyrpc.hashrate(function (res) {
                    if (res.error) return done(res);
                    assert(parseInt(res) >= 0);
                    done();
                });
            ...
            it("mining", function (done) {
                fourtwentyrpc.mining(function (res) {
                    if (res.error) return done(res);
            ...
            it("clientVersion", function (done) {
                fourtwentyrpc.clientVersion(function (res) {
                    if (res.error) return done(res);
                    assert.isString(res);
                    assert.strictEqual(res.split('/')[0], "G420");
                    done();
                });
            });
        });
    ...
    describe("listeners", function () {
        ...
        describe("stderr", function () {
            ...
            var test = function (t) {
                ...
                it("g420.stderr(" + label + ")", function (done) {
                    this.timeout(TIMEOUT);
                    g420.stop(function () {
                        g420.start(options, {
                            stderr: function (data) {
                                if (g420.debug) process.stdout.write(data);
                            ...
                            if (err) {
                                return g420.stop(function () { done(err); });
                            }
                            if (!spawned) {
                                return g420.stop(function () { done(new Error("where's the g420?")); });
                            }
                            g420.stderr(t.label, function (data) {
                                if (g420.debug) process.stdout.write(data);
